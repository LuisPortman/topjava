...
@NamedQueries({
        ...
        @NamedQuery(name = Meal.BETWEEN_INCLUSIVE, query = "SELECT m FROM Meal m WHERE m.user.id=:user_id AND m.dateTime >=: start_date AND m.dateTime <: end_date ORDER BY m.dateTime DESC")
})
@Entity
@Table(name = "meals", uniqueConstraints = {@UniqueConstraint(columnNames = {"user_id", "date_time"}, name = "meals_unique_user_time_idx")})
